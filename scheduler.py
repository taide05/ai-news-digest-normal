...
def start_scheduler(db_conn, cfg, orchestrator_module):
    ...
    if cfg.scheduler.run_on_startup:
        ...
        try:
            # Note: startup collection runs the pipeline directly (not via daily_job)
            # to get immediate data. Historical snapshots are only generated by cron
            # to avoid creating empty/minimal snapshots on first boot.
